...
We use SQLAlchemy ORM which lets us define database tables as Python classes.
Each class = one table. Each attribute = one column.

Tables:
  - users              → All users (admin, student, company) in one table
  - student_profiles   → Extra info for students (1-to-1 with users)
  - company_profiles   → Extra info for companies (1-to-1 with users)
  - placement_drives   → Job drives posted by companies
...
# ──────────────────────────────────────────────────────────────────
# User Model — single table for all three roles
# ──────────────────────────────────────────────────────────────────
class User(db.Model):
    __tablename__ = 'users'
    ...
    # One-to-one relationships with profile tables
...
# ──────────────────────────────────────────────────────────────────
# Student Profile — extra details for students
# ──────────────────────────────────────────────────────────────────
class StudentProfile(db.Model):
    ...
    department      = db.Column(db.String(100))  # e.g. "CS", "IT", "EC"
    cgpa            = db.Column(db.Float, default=0.0)
    graduation_year = db.Column(db.Integer)
    phone           = db.Column(db.String(20))
    resume_path     = db.Column(db.String(256))  # filename of uploaded resume
    ...
    def __init__(self, user_id: int, full_name: str = '', department: str = '',
        ...
        """Explicit constructor to satisfy static type checkers (Pylance/mypy).
        SQLAlchemy's instrumented __init__ handles actual column mapping at runtime."""
...
# ──────────────────────────────────────────────────────────────────
# Company Profile — extra details for companies
# ──────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────
# Placement Drive — a job opening posted by a company
# ──────────────────────────────────────────────────────────────────
class PlacementDrive(db.Model):
    ...
    # Stored as JSON string: {"min_cgpa": 7.0, "branches": ["CS","IT"], "grad_years": [2025,2026]}
...
# ──────────────────────────────────────────────────────────────────
# Application — a student applying to a placement drive
# ──────────────────────────────────────────────────────────────────
class Application(db.Model):
    ...
    # Status values: applied → shortlisted → selected/rejected/waiting

    # This constraint prevents a student from applying twice to the same drive
    __table_args__ = (
